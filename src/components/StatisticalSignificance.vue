<template>
	<div>
		...
		<v-layout row>
			...
			<v-flex class="" xs7 sm7 md7>
				<v-card>
					<v-card-text>
						<h3>С вероятностью</h3>
						<ul>
							<li>70% &mdash; конверсия находится между <strong>{{interval70[0]}}%</strong> и <strong>{{interval70[1]}}%</strong></li>
...
<script>
	...
	export default {
		...
		computed: {
			interval70() {
				return getInterval(this.nCount, this.convCount / this.nCount, 1.036)
			},
			interval80() {
				return getInterval(this.nCount, this.convCount / this.nCount, 1.282)
			},
			interval90() {
				return getInterval(this.nCount, this.convCount / this.nCount, 1.645)
			},
			interval95() {
				return getInterval(this.nCount, this.convCount / this.nCount, 1.96)
			},
			interval99() {
				return getInterval(this.nCount, this.convCount / this.nCount, 2.57)
			}
		},
		created() {
	...
	function getInterval(n, p, nSigma) {
		...
		return [(minConv * 100).toFixed(2).replace(/([0-9]+(\.[0-9]+[1-9])?)(\.?0+$)/, '$1'), (maxConv * 100).toFixed(2).replace(/([0-9]+(\.[0-9]+[1-9])?)(\.?0+$)/, '$1')]
	}
</script>
